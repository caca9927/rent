...
<body>
    ...
    <nav class="navbar navbar-expand-sm bg-dark navbar-dark">
        ...
        <a class="navbar-brand account" href="./user.php">
            <img src="<?= "img/" . $rs['profile_pic']; ?>" style="width:40px;border-radius: 50%;">&nbsp;
            <?php echo $rs['name']; ?>
        </a>
        <!-- Links -->
        <ul class="navbar-nav">
    ...
    <div class="inside">
        ...
        ?>
        <h2 style="text-align: center;"><?= $rs['apart_name'] ?> <div class="visitors">ผู้เข้าชม : <?= $rs['visitors'] ?></div>
        </h2>
    ...
    <div class="comment">
        ...
        <form action="./comment_user.php" method="post">
            ...
            <div class="form-group">
                <div class="row">
                    <div class="col">
                        <input type="text" class="form-control" placeholder="ชื่อ" name="name" value="<?= $row['name']; ?>" disabled>
                    </div>
                    <div class="col">
                        <input type="text" class="form-control" placeholder="เบอร์โทรศัพท์" name="phone" value="<?= $row['phone']; ?>" disabled>
                    </div>
                    <div class="col">
        ...
        ?>
            <div class="data_comment">
                <div class="name_phone">
                    <?= $row['name'] ?> (<?= $row['phone'] ?>) <i><?= $row['date_time'] ?></i> <a <?php

                                                                                                    $check_id = "SELECT * FROM aparment WHERE id = $aid";
                                                                                                    $query_check_id = mysqli_query($conn, $check_id);
                                                                                                    $getObj = mysqli_fetch_array($query_check_id);

                                                                                                    if ($getObj['user_id'] != $uid) {
                                                                                                        echo "hidden";
                                                                                                    } else {
                                                                                                        echo " ";
                                                                                                    }

                                                                                                    ?> onClick="javascript: return confirm('ยืนยันการลบ');" href="http://127.0.0.1/rent/delete_comment.php?cid=<?= $row['id_com'] ?>"><i class="material-icons">delete_forever</i></a>
                </div>
                <?= $row['comment']; ?> <br>
                <div class="data_reply_main" <?php
                                                $id_com = $row['id_com'];
                    ...
                    ?>
                        ผู้ดูแลที่พัก : <?= $re['reply'] ?> <i><?= $re['date_time'] ?></i> <a <?php
                                                                                                ...
                                                                                                ?> onClick="javascript: return confirm('ยืนยันการลบ');" href="http://127.0.0.1/rent/delete_reply.php?rid=<?= $re['id_reply'] ?>"><i class="material-icons">delete_forever</i></a> <br>
                    <?php } ?>
                </div>
                <div class="reply" <?php
                    ...
                    <form action="./reply.php" method="post">
                        <div class="form-group">
                            <textarea class="form-control" rows="2" placeholder="ตอบกลับ..." name="reply"></textarea>
                        </div>
                        <div class="form-group">
                            <div class="row">
                                <div class="col">
                                    <input type="text" value="<?= $row['id_com'] ?>" name="id_com" hidden>
                                    <button type="submit" class="btn btn-primary">ส่งข้อความ</button>
                                </div>
                            </div>
                        </div>
